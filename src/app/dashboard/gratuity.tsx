...
export default function Gratuity() {
  ...
  const calculateGratuityPay = (startDate: string): number => {
    ...
    if (monthsWorked >= 4) gratuityAmount = 7000;
    else if (monthsWorked >= 3) gratuityAmount = 6000;
    else if (monthsWorked >= 2) gratuityAmount = 5000;
    else if (monthsWorked >= 1) gratuityAmount = 4000;
    else gratuityAmount = 0;
  ...
  const handleCalculate = () => {
    const newData = data.map((row) => ({
      ...row,
      totalIncentive: row.startDate ? calculateGratuityPay(String(row.startDate)) : 0, 
    }));
    setData(newData);
          ...
          <div className="flex items-center gap-2 px-4">
            ...
            <Breadcrumb>
              <BreadcrumbList>
                ...
                <BreadcrumbItem>
                  <BreadcrumbPage>Gratuity calculator</BreadcrumbPage>
                </BreadcrumbItem>
              </BreadcrumbList>
            </Breadcrumb>
          </div>
          ...
          <div className="sm:flex sm:items-center sm:justify-between">
            <div>
              <div className="flex items-center gap-x-3">
                <h2 className="text-lg font-medium text-gray-800 dark:text-white">Employee Gratuity</h2>
                <span className="px-3 py-1 text-xs text-blue-600 bg-blue-100 rounded-full dark:bg-gray-800 dark:text-blue-400">
                  {data.length > 0 ? `${data.length} employees` : "0"}
                </span>
              </div>
              <p className="mt-1 text-sm text-gray-500 dark:text-gray-300">Upload employee data and calculate Gratuity incentives.</p>
            </div>
            ...
            <div className="flex items-center mt-4 gap-x-3">
              ...
              <button
                className={`flex items-center justify-center w-1/2 px-5 py-2 text-sm tracking-wide text-white transition-colors duration-200 rounded-md ${
                  isCalculated
                    ? "bg-green-500 hover:bg-green-600 dark:hover:bg-green-500 dark:bg-green-600"
              ...
              <table className="min-w-full">
                <thead>
                  <tr className="bg-gray-100 dark:bg-gray-800">
                    <th className="px-4 py-2 text-sm font-medium text-left text-gray-700 dark:text-gray-300 w-1/3">Name</th>
                    <th className="px-4 py-2 text-sm font-medium text-left text-gray-700 dark:text-gray-300 w-1/3">Start Date</th>
                    <th className="px-4 py-2 text-sm font-medium text-left text-gray-700 dark:text-gray-300 w-1/3">Total Incentive</th>
                  </tr>
                </thead>
                <tbody>
                  {data.length === 0 ? (
