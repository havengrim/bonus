...
export default function Page() {
  ...
  const calculateSRI = (startDate: string): number => {
    ...
    if (monthsWorked >= 4) sriAmount = 20000;
    else if (monthsWorked >= 3) sriAmount = 8000;
    else if (monthsWorked >= 2) sriAmount = 6000;
    else if (monthsWorked >= 1) sriAmount = 4000;
    else sriAmount = 2000;
  ...
  const handleCalculate = () => {
    const newData = data.map((row) => ({
      ...row,
      totalIncentive: row.startDate ? calculateSRI(String(row.startDate)) : 0, 
    }));
    setData(newData);
          ...
          <div className="sm:flex sm:items-center sm:justify-between">
            <div>
              <div className="flex items-center gap-x-3">
                <h2 className="text-lg font-medium text-gray-800 dark:text-white">Employee SRI</h2>
                <span className="px-3 py-1 text-xs text-blue-600 bg-blue-100 rounded-full dark:bg-gray-800 dark:text-blue-400">
                  {data.length > 0 ? `${data.length} employees` : "0"}
                </span>
              </div>
              <p className="mt-1 text-sm text-gray-500 dark:text-gray-300">Upload employee data and calculate SRI incentives.</p>
            </div>
